...
let store_descr fd = Unix_util.int_of_file_descr fd (* this can be an hashtable in more complicated cases *)
let retrieve_descr i = Unix_util.file_descr_of_int i

let xmp_read path buf offset d = 
  let hnd = retrieve_descr d in
    ignore (LargeFile.lseek hnd offset SEEK_SET);
    Unix_util.read hnd buf

let xmp_write path buf offset d =
  let hnd = retrieve_descr d in
    ignore (LargeFile.lseek hnd offset SEEK_SET);
...
let _ =
  main Sys.argv 
	...
	readdir = (fun path hnd -> "."::".."::(Array.to_list (Sys.readdir path)));
