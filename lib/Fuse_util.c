...
void ml2c_Unix_struct_statfs(value v,struct statvfs * st)
{
  memset(st,0,sizeof(*st));
  st->f_bsize = Int64_val(Field(v,0));
  st->f_blocks = Int64_val(Field(v,1));
  st->f_bfree = Int64_val(Field(v,2));
  st->f_bavail = Int64_val(Field(v,3));
  st->f_files = Int64_val(Field(v,4));
  st->f_ffree = Int64_val(Field(v,5));
  st->f_namemax = Int64_val(Field(v,6));

  /* TODO: check the meaning of the following missing fields:
